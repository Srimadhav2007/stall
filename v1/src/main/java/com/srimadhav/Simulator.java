...
public class Simulator {
    ...
    static int memsize, l1is, l1ib, l1ds, l1db, l2s, l2b, bsize;
    ...
    static void initCacheUI(JPanel panel, JLabel[][][] labels, int sets, int blocks, int bsize) {
        ...
        for (int s = 0; s < sets; s++) {
            ...
            setPanel.setBorder(
                    BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY), "Set " + s));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            l2s = json.getInt("sets2");
            l2b = json.getInt("l2") / (l2s * bsize);
        } catch (Exception e) {
        ...
        statsPanel.setBorder(
                BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY), "Execution Stats"));
        ...
        JLabel[] statLabels = { clockCyclesLbl, exStallsLbl, memStallsLbl, instrLbl, ipcLbl };
        for (JLabel lbl : statLabels) {
        ...
        addrHdr.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.GRAY),
                BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            bHdr.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.GRAY),
                    BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        ...
        for (int i = 0; i < memsize; i += 4) {
            ...
            addrLabel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.GRAY),
                    BorderFactory.createEmptyBorder(5, 5, 5, 5)));
            ...
            for (int j = 0; j < 4; j++) {
                if (i + j < memsize) {
                    ...
                    valLabel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.GRAY),
                            BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        ...
        JButton cbl2 = new JButton("L2");

        JButton[] cacheBtns = { cbl1i, cbl1d, cbl2 };
        ...
        l2Pane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);

        l1iPane.getVerticalScrollBar().setUnitIncrement(16);
        l1dPane.getVerticalScrollBar().setUnitIncrement(16);
        l2Pane.getVerticalScrollBar().setUnitIncrement(16);

        cacheCards.add(l1iPane, "L1I");
        cacheCards.add(l1dPane, "L1D");
        cacheCards.add(l2Pane, "L2");

        cbl1i.addActionListener(e -> {
            cacheLayout.show(cacheCards, "L1I");
            cacheCards.revalidate();
            cacheCards.repaint();
        });
        cbl1d.addActionListener(e -> {
            cacheLayout.show(cacheCards, "L1D");
            cacheCards.revalidate();
            cacheCards.repaint();
        });
        cbl2.addActionListener(e -> {
            cacheLayout.show(cacheCards, "L2");
            cacheCards.revalidate();
            cacheCards.repaint();
        });
        ...
        b1.addActionListener(e -> {
            pLayout.show(mainPanel, "Program");
            mainPanel.revalidate();
            mainPanel.repaint();
        });
        b2.addActionListener(e -> {
            pLayout.show(mainPanel, "Registers");
            mainPanel.revalidate();
            mainPanel.repaint();
        });
        b3.addActionListener(e -> {
            pLayout.show(mainPanel, "Memory");
            mainPanel.revalidate();
            mainPanel.repaint();
        });
        b4.addActionListener(e -> {
            pLayout.show(mainPanel, "Cache");
            mainPanel.revalidate();
            mainPanel.repaint();
        });
        ...
        Runnable applyLayout = () -> {
            ...
            statsPanel.setBounds(23 * w / 25, 4 * h / 15 + h / 30, w - 23 * w / 25, 7 * (h / 30));
        ...
        runBtn.addActionListener(e -> {
            ...
            try {
                ...
                // l2
                JSONObject l2Json = json.getJSONObject("l2");
                JSONArray l2Sets = l2Json.getJSONArray("sets");
                for (int s = 0; s < l2s; s++) {
                    JSONArray set = l2Sets.getJSONArray(s);
                    for (int b = 0; b < l2b; b++) {
                        JSONArray bytes = set.getJSONObject(b).getJSONArray("bytes");
                        for (int i = 0; i < bsize; i++) {
                            l2Labels[s][b][i].setText(String.format("%02X", bytes.getInt(i)));
                        }
                    }
                }
